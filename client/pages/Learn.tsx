    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((i) => (
          <article
            key={i.id}
            className="rounded-xl border border-border/60 bg-card-gradient p-5 hover:shadow-md transition"
          >
        ...
        <div className="grid gap-3">
          <div className="flex gap-2 text-sm">
            <button
              ...
              className={`px-3 py-1.5 rounded-md ${type === "blog" ? "btn-gradient text-black" : "border border-white/15"}`}
            >
              Blog
            </button>
            <button
              onClick={() => setType("course")}
              className={`px-3 py-1.5 rounded-md ${type === "course" ? "btn-gradient text-black" : "border border-white/15"}`}
            >
              Course
            </button>
          </div>
