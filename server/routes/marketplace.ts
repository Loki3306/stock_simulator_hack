...
marketplaceRouter.get("/", async (_req, res) => {
  const items = await MarketplaceItem.find({ isPublished: true }).sort({ createdAt: -1 }).limit(100);
  res.json(items);
});

marketplaceRouter.post("/publish", authenticate as any, async (req: any, res) => {
  ...
  const item = await MarketplaceItem.findOneAndUpdate(
    { strategyId },
    { authorId: req.user.id, title: title || s.title, price, tags, isPublished: true },
...
marketplaceRouter.post("/import/:id", authenticate as any, async (req: any, res) => {
